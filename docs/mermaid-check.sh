...
#
# Usage:
#   docs/mermaid-check.sh                       # scan CWD recursively
#   docs/mermaid-check.sh interview/            # scan one directory
#   docs/mermaid-check.sh interview/DESIGN.md   # scan one file
#   docs/mermaid-check.sh -j 8 interview/       # 8 parallel validators
#
...
export SVG WORK MMDC
validate_block() {
  local src="$1" idx="$2" mmd="$3" err base
  base="$(basename "$mmd" .mmd)"
  if err="$(npx -y @mermaid-js/mermaid-cli -i "$mmd" -o "$SVG/$base.svg" 2>&1 >/dev/null)"; then
    printf 'ok    %s  #%s\n' "$src" "$idx"
    return 0
  fi
  printf 'FAIL  %s  #%s\n' "$src" "$idx"
